// Physical memory allocator, for user processes,
// kernel stacks, page-table pages,
// and pipe buffers. Allocates whole 4096-byte pages.

#include "types.h"
#include "param.h"
#include "memlayout.h"
#include "spinlock.h"
#include "riscv.h"
#include "defs.h"

void freerange(void *pa_start, void *pa_end);

extern char end[]; // first address after kernel.
                   // defined by kernel.ld.

struct run {
  struct run *next;
};

struct {
  struct spinlock lock;
  struct run *freelist;
} kmem;

char *pagebase;
#define REFPERPAGE ((PGSIZE - sizeof(struct spinlock)) / sizeof(char))
#define REFLOCK(pa) ((struct spinlock *)PGROUNDDOWN(pa))
#define REFONPAGE(page, idx) (((char *)(page) + sizeof(struct spinlock)) + idx)

char addpageref(void *pa, char c)
{
  if (((uint64)pa % PGSIZE) != 0 || (char *)pa < end || (uint64)pa >= PHYSTOP)
    panic("addpageref");
  uint64 idx = ((char *)pa - pagebase) / PGSIZE;
  uint64 pgn = idx / REFPERPAGE;
  idx = idx % REFPERPAGE;
  char *refcnt = REFONPAGE(pagebase + pgn * PGSIZE, idx);
  acquire(REFLOCK((uint64)refcnt));
  char ret = *refcnt += c;
  release(REFLOCK((uint64)refcnt));
  return ret;
}

void
kinit()
{
  initlock(&kmem.lock, "kmem");
  freerange(end, (void*)PHYSTOP);
}

void
freerange(void *pa_start, void *pa_end)
{
  pagebase = (char *)PGROUNDUP((uint64)pa_start);
  int size = ((char *)pa_end - pagebase) / PGSIZE;
  char *p = pagebase;
  for (int i = 0; i < size; i += REFPERPAGE)
  {
    memset(p, 1, PGSIZE);
    initlock((struct spinlock *)p, "page refcnt");
    p += PGSIZE;
  }
  for (; p + PGSIZE <= (char *)pa_end; p += PGSIZE)
    kfree(p);
}

// Free the page of physical memory pointed at by pa,
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(void *pa)
{
  struct run *r;

  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    panic("kfree");

  if (addpageref(pa, -1))
    return;

  // Fill with junk to catch dangling refs.
  memset(pa, 1, PGSIZE);

  r = (struct run*)pa;

  acquire(&kmem.lock);
  r->next = kmem.freelist;
  kmem.freelist = r;
  release(&kmem.lock);
}

// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
void *
kalloc(void)
{
  struct run *r;

  acquire(&kmem.lock);
  r = kmem.freelist;
  if(r)
    kmem.freelist = r->next;
  release(&kmem.lock);

  if (r)
  {
    memset((char*)r, 5, PGSIZE); // fill with junk
    addpageref((void *)r, 1);
  }
  return (void*)r;
}
